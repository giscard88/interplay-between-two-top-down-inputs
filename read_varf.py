...
for xin in range(1,21):
    fp=open('varf/weights_super_ON_deep_ON_'+str(xin)+'_'+test_f+'.json','r')
    data=json.load(fp)
    temp=data['after_p_p']
    ...
    fp=open('varf/weights_super_OFF_deep_ON_'+str(xin)+'_'+test_f+'.json','r')
    data=json.load(fp)
    temp=data['after_p_p']
    temp=numpy.array(temp)-w_bu_pyr
    ...
    fp=open('varf/weights_super_ON_deep_OFF_'+str(xin)+'_'+test_f+'.json','r')
    data=json.load(fp)
    temp=data['after_p_p']
    temp=numpy.array(temp)-w_bu_pyr
...
pylab.legend()
pylab.savefig('/local2/STDP_prune_refined/figs/read_varf_1.eps')
...
pylab.legend()
pylab.savefig('/local2/STDP_prune_refined/figs/read_varf_2.eps')
